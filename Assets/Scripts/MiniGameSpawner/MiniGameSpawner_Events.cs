...
public partial class MiniGameSpawner
{
    ...
    {
        if (!gate.TryEnter("점검시간 상태라 클리어 이벤트 무시"))
        {
            return;
        }
        ...
        gate.ShowSuccess(gameClockTimer);
    }

    private void HandleGameOver()
    {
        if (!gate.TryEnter("점검시간 상태라 실패 이벤트 무시"))
        {
            return;
        }
    ...
    {
        if (!gate.TryEnter("점검시간 상태라 외부 게임오버 무시"))
        {
            return;
        }
    ...
    public void ForceGameOver()
    {
        ...
        gate.ShowMaintenance(gameClockTimer);
    }

    public void ShowTimeoutPopup()
    {
        if (!gate.TryEnter("점검시간 상태라 장시간 응답 없음 팝업 무시"))
        {
            return;
        }
        ...
        gate.ShowTimeout(gameClockTimer);
    }

    public void ShowMeaningErrorPopup()
    {
        if (!gate.TryEnter("점검시간 상태라 의미 불명 팝업 무시"))
        {
            return;
        }
        ...
        gate.ShowMeaningError(gameClockTimer);
    }
}
